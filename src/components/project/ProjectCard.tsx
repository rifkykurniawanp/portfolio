...
export default function ProjectCard({ project, onPreview }: ProjectCardProps) {
  return (
    <div className={cn(
      "group relative overflow-hidden rounded-[2rem]", // Extra rounded
      "border border-border/40 bg-card/40 backdrop-blur-sm", // Minimalist border
      "transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-[#5227FF]/5 hover:border-border/80"
    )}>
      {/* Image Container */}
      <div className="relative overflow-hidden h-52 m-2 rounded-[1.6rem]"> 
        <Image
          src={project.image}
          alt={project.title}
          fill
